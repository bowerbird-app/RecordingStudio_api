...
class CursorInstallTest < Minitest::Test
  ...
  def test_environment_json_points_dummy_rails_and_tailwind_terminals
    env = JSON.parse(File.read(File.join(ROOT, ".cursor/environment.json")))
  ...
  def test_gemspec_excludes_cursor_boot_files
    spec = Gem::Specification.load(File.join(ROOT, "recording_studio_api.gemspec"))
    cursor_files = spec.files.grep(%r{\A\.cursor/})
