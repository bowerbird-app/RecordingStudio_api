...
module OperationsApi
  class ScalarTestCredentialsController < ApplicationController
    include OperationsApi::ScalarTestAuth
    ...
    def create
      ...
      result = RecordingStudioApi::Services::IssueTestCredential.call(
        ...
        role: params[:role],
        name: "Operations API #{params[:role]} test token"
      )
      ...
      session[scalar_test_auth_notice_key] = "Operations API test bearer token issued."
    ...
    def destroy
      ...
      session[scalar_test_auth_notice_key] = "Operations API test bearer token revoked."
      redirect_to scalar_test_auth_return_path
    end
