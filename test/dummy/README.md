...
- Delegated OAuth consent (`/recording_studio_api/oauth/authorize`) and a dummy connected-apps list; Accessible 0.8 dependent grants (`depends_on_recording_id`) cap each connect by the manager Access recording. Connect is two screens: pick the Access you grant from (including a Folder, labeled with that parent’s name — not a role; staff AdminRoot is omitted), then permission and Continue.
