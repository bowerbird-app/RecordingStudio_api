...
- Delegated OAuth consent (`/recording_studio_api/oauth/authorize`) and a dummy connected-apps list; Accessible 0.8 dependent grants (`depends_on_recording_id`) cap each connect by the manager Access recording. Connect is two screens: pick the Access you grant from (including a Folder, labeled with that parent’s name — not a role; staff AdminRoot is omitted), then permission and Continue. Screen 1 trailing copy is Connect / Connected / Reconnect.
...
Authenticated pages include `RecordingStudio::UsesDefaultLayout` and render the gem's `recording_studio/default_layout` (theme `rounded` on `body`; dummy also stamps `data-theme="rounded"` on `html`). API-key, consent, and connected-apps screens do not render the root-switch dropdown in the page-nav slot. Consent and connected apps sit in the first cell of a two-column Flatpack Grid. Consent is two screens titled `Connect {app}`: Screen 1 is a Flatpack List inside a Card of Access they can grant from (workspace or folder parent names, same List item for every row, quiet trailing Connect / Connected / Reconnect; staff AdminRoot is omitted). The Card hugs the list. Screen 2 is permission (when there is more than View) plus stacked Continue / Cancel. Connected apps uses a Flatpack List inside a Card. The home page in `app/views/home/standard_root.html.erb` stays intentionally small and links into this gem's API key screens and connected apps. Seeds include a second workspace, folder-level Access on Product Docs (a normal Connect row, not nested), `folder@example.com` (folder Access only), a public OAuth app (`rsapi_oc_seed_demo_app`), one connected grant, and one voided grant for Reconnect. Staff AdminRoot Access stays for dummy admin screens and is not a Connect row. Use the dedicated docs pages for deeper install, config, auth, and API route notes.
