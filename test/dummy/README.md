...
- Delegated OAuth consent (`/recording_studio_api/oauth/authorize`) and a dummy connected-apps list; Accessible 0.8 dependent grants (`depends_on_recording_id`) cap each connect by the manager Access recording. Connect is two screens: pick the Access you grant from (including a Folder), then permission and Continue.
