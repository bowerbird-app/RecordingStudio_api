...
module RecordingStudioApi
  class AdminDashboardsController < AdminController
    def show
      ...
      errors_daily_points = build_log_chart_points(
        buckets: daily_buckets(30),
        scope: errors_logs_scope
      ) do |occurred_at|
        occurred_at.in_time_zone.beginning_of_day
      end
      @errors_chart_categories_last_30_days = errors_daily_points.map { |point| point[:time].strftime("%b %-d") }
      @errors_chart_series_last_30_days = errors_daily_points.map { |point| point[:count] }
      @errors_volume_last_30_days = errors_daily_points.sum { |point| point[:count] }
