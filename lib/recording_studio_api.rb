...
require "recording_studio_api/registered_action_context"
...
module RecordingStudioApi
  class << self
    ...
    def register_action(name, api: :public, http_verb:, path:, handler:, serializer: nil, openapi: nil, input_contract: nil)
      configuration.api(api).registered_action_registry.register(
        name,
        http_verb: http_verb,
        path: path,
        handler: handler,
    ...
    def registered_action(name, api: :public)
      configuration.fetch_api(api).registered_action_registry[name]
    end

    def registered_action_request_match(request)
      ...
      configuration.fetch_api(api_key).registered_action_registry.match_path(path)
