...
- Consent and connected-app screens composed from `recording_studio/default_layout` + FlatPack (`UsesDefaultLayout`); host authentication (dummy Devise) owns sign-in. Consent is two screens in the first cell of `FlatPack::Grid` (`cols: 2`): Screen 1 lists Access they can grant from (including a Folder) as a Flatpack List in a Card, labeled with that parent’s name — not a role — with Connected / Reconnect; staff AdminRoot is omitted. Screen 2 is permission then grant (`Connect {app}`), with permission only when there is more than View (default `view`) and stacked full-width Continue / Cancel. Connected apps uses a Flatpack List inside a Card.
