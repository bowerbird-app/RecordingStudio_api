...
- Dummy Accessible setup configures `access_actor_types` (including `RecordingStudioApi::ApiClient`) and drops legacy `AllowsAccessibleChildren` usage
- `ProvisionApiClient` persists the API client before `grant_access` so Accessible 0.5+ actor persistence checks succeed
- `ApiClient#access_recording` is optional at create time so that provision order works; it is set before the provision transaction commits
- Access-management authorization for provisioning and role assignment uses recording-level Accessible checks so a stronger grant on a descendant cannot manage a weaker access point

### Upgrade notes
- See [UPGRADING.md](UPGRADING.md) for Redis 6, Accessible 0.5, and the API-client access-recording nullability migration
